	...
	factory: (require) => {
		...
		* the viewport so the fullscreen expansion can fill the window). */
		function setWidth(state, width) {
			// [stage B 2026-08-31 -> stage B2 2026-08-31] cap follows the
			// activity-rail width (--ssb-rail-width, 48 or 200px).
			const max = typeof window !== "undefined" ? Math.max(280, window.innerWidth - ssbRailWidthPx()) : 640;
			return {
				...state,
		...
		*/
		function sanitizeState(parsed) {
			...
			// [stage B2 2026-08-31] hydration cap follows the rail width.
			const maxWidth = typeof window !== "undefined" ? window.innerWidth - ssbRailWidthPx() : Infinity;
			return {
		...
		*/
		...
		let boundModules;
		function bindModuleSystem(mod) {
			boundModules = mod;
		}
		/** Resolve the shell-installed module system: context-bound (rc.8+) or the legacy page global (rc.6). */
		...
		*/
		...
		var sidebar_module_css_default = {
			...
			"splitPositioned": "nArs4W_splitPositioned",
			"panelEdgeJunction": "nArs4W_panelEdgeJunction",
			"junctionHandle": "nArs4W_junctionHandle",
			"junctionHandleActive": "nArs4W_junctionHandleActive",
		...
		*/
		...
		/**
		* [stage A 2026-08-31] Left-panel toggle glyph (the "左侧栏" button): a
		* frame with a filled strip along its LEFT edge — exact mirror of
		* IconPanelRightOutline16 so the three zone toggles read as one family.
		*/
		const IconPanelLeftOutline16 = ({ size = 16, className }) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
			width: size,
			height: size,
			className,
			viewBox: "0 0 16 16",
		...
		* move and the divider would run away from the cursor. */
		...
		/** fork: 4-way junction handle — where a split's divider meets an
		 * opposite-direction nested split's divider (three panes meeting at
		 * one point), a small circular `cursor: move` grip lets one drag feed
		 * BOTH resizeSplit routes: x delta → the outer split, y delta → the
		 * nested one (mirrored for column splits). Deltas stay incremental
		 * like Divider for the same run-away-prevention reason. */
		function JunctionHandle(props) {
			const { style, onResizeX, onResizeY } = props;
			const last = (0, react.useRef)({ x: 0, y: 0, w: 1, h: 1 });
			const [dragging, setDragging] = (0, react.useState)(false);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: clsx(sidebar_module_css_default.junctionHandle, dragging && sidebar_module_css_default.junctionHandleActive),
				style,
				"aria-hidden": "true",
				onPointerDown: (event) => {
					event.preventDefault();
					event.stopPropagation();
					event.currentTarget.setPointerCapture(event.pointerId);
					/* handle → junction layer → split container box (for px→frac). */
					const box = event.currentTarget.parentElement?.parentElement?.getBoundingClientRect();
					last.current = {
						x: event.clientX,
						y: event.clientY,
						w: box === void 0 ? 1 : box.width,
						h: box === void 0 ? 1 : box.height
					};
					setDragging(true);
				},
				onPointerMove: (event) => {
					...
					if (dx !== 0) onResizeX(dx / Math.max(1, last.current.w));
					if (dy !== 0) onResizeY(dy / Math.max(1, last.current.h));
					last.current.x = event.clientX;
					last.current.y = event.clientY;
				},
				onPointerUp: (event) => {
		...
		/** fork: junction handles for one divider of a split node. `neighbor` is
		 * the adjacent child that is an opposite-direction split; each of its
		 * internal dividers ends exactly on this node's divider, producing one
		 * 4-way handle per crossing. */
		function SplitJunctions(props) {
			const { node, index, neighbor, actions } = props;
			const total = node.sizes.reduce((sum, size) => sum + size, 0);
			if (total <= 0) return null;
			const prefix = node.sizes.slice(0, index).reduce((sum, size) => sum + size, 0);
			const mainFrac = prefix / total;
			const nestedTotal = neighbor.sizes.reduce((sum, size) => sum + size, 0);
			if (nestedTotal <= 0) return null;
			const isRow = node.dir === "row";
			const handles = [];
			let acc = 0;
			for (let k = 0; k < neighbor.children.length - 1; k++) {
				acc += neighbor.sizes[k];
				handles.push(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(JunctionHandle, {
					style: isRow ? { left: `${mainFrac * 100}%`, top: `${acc / nestedTotal * 100}%` } : { top: `${mainFrac * 100}%`, left: `${acc / nestedTotal * 100}%` },
					onResizeX: isRow ? (deltaFrac) => {
						actions.resizeSplit(node.id, index - 1, deltaFrac);
					} : (deltaFrac) => {
						actions.resizeSplit(neighbor.id, k, deltaFrac);
					},
					onResizeY: isRow ? (deltaFrac) => {
						actions.resizeSplit(neighbor.id, k, deltaFrac);
					} : (deltaFrac) => {
						actions.resizeSplit(node.id, index - 1, deltaFrac);
					}
			...
			return handles.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(react_jsx_runtime.Fragment, { children: handles }) : null;
		}
		/** fork: shell-edge dividers — horizontal (col-split) dividers inside the
		 * workbench whose span touches the panel's LEFT edge. Each crossing of
		 * such a divider with the panel edge is a conversation|panel×pane
		 * junction that gets a 4-way handle: x → panel width (like the edge
		 * strip), y → that split's resizeSplit. Walk: col splits lay children
		 * full-width (recurse every child with its y-window); row splits only
		 * expose children[0] to the left edge. */
		function edgeDividerFracs(node, top, height, out) {
			if (node === null || node === void 0 || node.kind !== "split") return out;
			if (node.dir === "col") {
				const total = node.sizes.reduce((sum, size) => sum + size, 0);
				if (total > 0) {
					let acc = 0;
					for (let k = 0; k < node.children.length - 1; k++) {
						acc += node.sizes[k];
						out.push({ splitId: node.id, index: k, topFrac: top + height * (acc / total) });
					}
					let before = 0;
					for (let i = 0; i < node.children.length; i++) {
						const childFrac = node.sizes[i] / total;
						edgeDividerFracs(node.children[i], top + height * before, height * childFrac, out);
						before += childFrac;
					}
				}
			} else edgeDividerFracs(node.children[0], top, height, out);
			return out;
		}
		...
		*/
		...
		function NodeView(props) {
			...
			const isRow = node.dir === "row";
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: clsx(sidebar_module_css_default.split, isRow ? sidebar_module_css_default.splitRow : sidebar_module_css_default.splitCol, sidebar_module_css_default.splitPositioned),
				...
				}), /* fork: three-pane junction — when either side of this divider
				 * is an opposite-direction split, add 4-way handles at the
				 * crossing points. */, index > 0 && (() => {
					const perpendicular = (candidate) => candidate && candidate.kind === "split" && candidate.dir !== node.dir ? candidate : null;
					const neighbor = perpendicular(node.children[index]) ?? perpendicular(node.children[index - 1]);
					return neighbor === null ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SplitJunctions, {
						node,
						index,
						neighbor,
						actions
		...
		* entirely — re-enabling them is the settings page's job. */
		...
		/**
		* The secondary-side-bar toolbar's stylesheet. Injected as its own
		* <style data-ssb="1"> tag — deliberately OUTSIDE the compiled css-module
		* strings (which are never hand-edited). Classes use the .ssb-* prefix;
		* buttons clone the old toggleButton spec (28px round, dsw tokens).
		*/
		const SSB_CSS = [
			// [stage B fix 2026-08-31] the three-zone switch bar is HORIZONTAL
			// (user review): a bare row pinned to the window's top-right, above
			// the vertical activity rail (which starts at top:44px).
			".ssb-root{position:fixed;top:4px;right:4px;z-index:45;display:flex;flex-direction:row;align-items:center;gap:4px}",
			"body[data-dsh-title-bar-compat] .ssb-root{top:calc(var(--dsh-title-bar-strip,40px) + 3px)}",
			// [stage B fix2 2026-08-31] the tray IS the visible three-zone bar:
			// horizontal row, no card chrome (the buttons must read as one
			// horizontal strip pinned to the window's top-right).
			// [stage C2->C3 2026-08-31] the three-zone bar is a BARE strip (no fill,
			// no border): the buttons themselves are the chrome (32x32, 6px radius).
			".ssb-tray{display:flex;flex-direction:row;align-items:center;width:auto;gap:4px;padding:4px;border-radius:8px}",
			// [stage C 2026-08-31] align to design-spec: the top-right icon buttons
			// are 32x32 with a 6px (--r-1p5) radius, not the old 28px round chrome.
			".ssb-btn{width:32px;height:32px;flex:none;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;justify-content:center;align-items:center;padding:0;display:flex;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out)}",
			".ssb-btn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}",
			".ssb-btn:disabled{opacity:.4;cursor:default}",
			".ssb-btn[data-active=\"1\"]{color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-interactive-bg-active)}",
			".ssb-sep{width:20px;height:1px;flex:none;background:var(--dsw-alias-border-l1);margin:2px 0}",
			".ssb-glyph{font:var(--dsw-font-xxs-strong-12);line-height:1}",
			// [stage B 2026-08-31] Permanent right-edge activity rail (VS Code style):
			// one shortcut per enabled preset tab, always visible. The right panel
			// and bottom panel give way to it via right:48px / JS -48px offsets.
			// [stage B2 2026-08-31] width is driven by --ssb-rail-width so the
			// expanded (200px) state and the give-way offsets stay in sync.
			// [shell header 2026-08-31] one shared anchor is the actual global
			// top-bar lower edge; the right workbench and activity rail must agree.
			":root{--dsh-global-header-bottom:48px}",
			".nArs4W_panel{top:var(--dsh-global-header-bottom)}",
			"body[data-dsh-title-bar-compat] .nArs4W_panel{top:calc(var(--dsh-title-bar-strip,40px) + 4px);padding-top:0}",
			".ssb-actrail{position:fixed;right:0;top:var(--dsh-global-header-bottom);bottom:0;width:var(--ssb-rail-width,48px);transition:width .15s var(--ds-ease-in-out);z-index:45;display:flex;flex-direction:column;align-items:center;gap:4px;padding:4px;box-sizing:border-box;overflow:hidden;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2)}",
			"body[data-dsh-title-bar-compat] .ssb-actrail{top:calc(var(--dsh-title-bar-strip,40px) + 4px)}",
			".ssb-actrail-btn{width:32px;height:32px;flex:none;position:relative;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;justify-content:center;align-items:center;padding:0;display:flex;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out)}",
			// [stage C3 2026-08-31] drag handle on the rail's left edge; dragging
			// disables the width transition so the rail follows the pointer.
			".ssb-actrail-resize{position:absolute;left:-4px;top:0;bottom:0;width:8px;cursor:col-resize;touch-action:none;z-index:1}",
			"body[data-dsh-ssb-rail-dragging] .ssb-actrail{transition:none}",
			// [stage B3 2026-08-31] expanded (hover) row: icon + text, aligned with
			// the left session rail's entry rows (design-spec: 36px height, 12px
			// left padding, 8px radius, 8px gap, 20px label line-height).
			".ssb-actrail-btn-row{width:100%;height:36px;padding:0 10px 0 12px;gap:8px;justify-content:flex-start}",
			".ssb-actrail-btn-row .ssb-actrail-label{flex:1;min-width:0;text-align:left;font-size:13px;line-height:20px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".ssb-actrail-btn:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}",
			".ssb-actrail-btn:disabled{opacity:.4;cursor:default}",
			".ssb-actrail-btn[data-active=\"1\"]{color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-interactive-bg-active)}",
			".ssb-actrail-btn[data-active=\"1\"]:before{content:\"\";position:absolute;left:-4px;top:7px;bottom:7px;width:2px;border-radius:1px;background:var(--dsw-static-blue-450,#4c8dff)}",
			// Narrow (<768px) drawer mode: the panel goes full-width (100vw), so
			// the rail hides itself here; the panel's right:48px give-way is
			// reverted in the panel stylesheet's own narrow media query.
			"@media (width<=767px){.ssb-actrail{display:none}}",
			"@media (prefers-reduced-motion:reduce){.ssb-btn,.ssb-actrail-btn{transition:none}}",
			// [compact band 2026-08-31] one shared 48px top band: the app left-sidebar
			// logo row, conversation title, 对话/轨迹 tabs, Session log, and three-zone
			// tray all share its bottom edge; every workspace starts there. These late
			// overrides flatten the core title-row wrapper without editing core code.
			"body .wSkVaW_header{box-sizing:border-box;display:flex;align-items:stretch;gap:16px;height:var(--dsh-global-header-bottom);padding:0 28px 0 20px}",
			"body .wSkVaW_titleRow{display:contents}",
			"body .wSkVaW_titleCluster{flex:0 1 auto;min-width:0;order:0}",
			"body .wSkVaW_tabs{display:flex;align-items:stretch;flex:none;gap:24px;margin-top:0;order:1}",
			"body .wSkVaW_headerUtilities{align-items:flex-end;flex:none;margin-left:auto;order:2}",
			"body .wSkVaW_tab{display:flex;align-items:center;padding-bottom:0}",
			"body .wSkVaW_tab:after{bottom:0}",
			"body .hHd-Xa_logoRow{height:48px;padding:10px 0 10px 4px}",
			// [compact band 2026-08-31] global-band icon controls share a 28px,
			// 6px-radius, borderless visual primitive; Session log stays accessible.
			"body .nL4_yW_sessionLogButton{position:relative;align-self:center;box-sizing:border-box;width:28px;height:28px;min-width:28px;padding:0;gap:0;border:none;border-radius:6px;background:transparent}",
			"body .nL4_yW_sessionLogButton span{position:absolute;width:1px;height:1px;margin:-1px;padding:0;border:0;clip:rect(0 0 0 0);clip-path:inset(50%);overflow:hidden}",
			"body .hHd-Xa_logoRow button{box-sizing:border-box;width:28px;height:28px;min-width:28px;border:none;border-radius:6px;background:transparent}"
		].join("\n");
		/**
		* [stage B2 2026-08-31] Current activity-rail width in px, read from the
		* --ssb-rail-width CSS variable (48px collapsed, 200px expanded). JS
		* give-way offsets (bottom-panel right, width caps, center push) use this
		* instead of a hardcoded 48 so they follow the rail's two states.
		*/
		function ssbRailWidthPx() {
			try {
				const v = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--ssb-rail-width"));
				return Number.isFinite(v) && v > 0 ? v : 48;
			} catch {
				return 48;
			}
		}
		/** Idempotent style injection: skip when a marker tag already exists. */
		function ensureSsbStyles() {
			if (document.querySelector("style[data-ssb=\"1\"]") !== null) return;
			const tag = document.createElement("style");
			tag.setAttribute("data-ssb", "1");
			tag.textContent = SSB_CSS;
			document.head.appendChild(tag);
		}
		/** Inline chevron for the fold/unfold button (currentColor, theme-following). */
		const SsbChevronIcon = ({ up = false }) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("svg", {
			viewBox: "0 0 16 16",
			width: 15,
			height: 15,
			fill: "none",
			...
			strokeLinejoin: "round",
			"aria-hidden": true,
			style: up ? { transform: "rotate(180deg)" } : void 0,
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", { d: "M4 6.5 8 10.5 12 6.5" })
		});
		/** Inline canvas icon: a panel with a play glyph (the webpreview wake affordance). */
		const SsbCanvasIcon = () => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
			viewBox: "0 0 16 16",
			width: 15,
			height: 15,
			fill: "none",
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("rect", {
				...
				d: "M6.6 6.2 10 8 6.6 9.8z",
				fill: "currentColor",
				stroke: "none"
			})]
		});
		/**
		* The secondary-side-bar tool rail: replaces the old top-right two-button
		* toggleCluster with three zones —
		*   A) canvas wake/close: reuses the exact service calls behind the
		*      message-header Play button (dsh-web-preview-panel's openPreviewTab /
		*      closePreviewTab): BetterSidebarService.openTab({type:"webpreview",
		*      path}) to wake/focus, closeTab to close when open AND active;
		*   B) one shortcut per enabled preset tab type (registry getTabs() ∩
		*      tabsEnabled, "agent:*" session terminals excluded): collapsed panel
		*      opens first, then openTab activates; clicking the already-active
		*      shortcut collapses the panel;
		*   C) the right-panel and bottom-panel toggles (semantics unchanged from
		*      the old cluster buttons).
		* Collapsed by default to a single chevron button; the expanded tray is
		* local UI state only (not persisted). In the no-session branch every
		* button renders disabled, matching the old early-return behavior.
		*/
		function SecondarySideBarToolbar(props) {
			const { ctx, store, state, sessionId, narrow, disabled = false } = props;
			(0, react.useEffect)(() => {
				ensureSsbStyles();
			}, []);
			// [stage B3 2026-08-31] hover-driven two-state rail, aligned with the
			// left session rail's collapse/expand UX: pointer enters -> the rail
			// widens to 200px and each shortcut shows icon + text rows; pointer
			// leaves -> back to the 48px icon-only rail. No toggle button and no
			// persistence; all give-way offsets follow --ssb-rail-width live.
			// [stage B3 -> C3 2026-08-31] drag-to-expand rail (user review): no
			// hover auto-expand — the rail stays a 48px icon rail and the user
			// drags its left edge to widen it into the icon+text rows. The width
			// is held here (persisted) and drives --ssb-rail-width, so all
			// give-way offsets track the dragged width live.
			const [railW, setRailW] = (0, react.useState)(() => {
				try { const v = Number(localStorage.getItem("ssb-rail-w")); return Number.isFinite(v) && v >= 48 && v <= 220 ? v : 48; } catch { return 48; }
			});
			const RAIL_MIN = 48, RAIL_MAX = 220;
			const railWRef = (0, react.useRef)(railW);
			railWRef.current = railW;
			const railDragRef = (0, react.useRef)({ on: false });
			const applyRailW = (0, react.useCallback)((w) => {
				const clamped = Math.min(RAIL_MAX, Math.max(RAIL_MIN, Math.round(w)));
				setRailW(clamped);
				return clamped;
			}, []);
			const onRailDragStart = (0, react.useCallback)((e) => {
				railDragRef.current.on = true;
				document.body.setAttribute("data-dsh-ssb-rail-dragging", "");
				const onMove = (ev) => {
					if (!railDragRef.current.on) return;
					// rail is fixed right:0, so the dragged left edge at ev.clientX
					// gives width = window.innerWidth - ev.clientX.
					applyRailW(window.innerWidth - ev.clientX);
				};
				const onUp = () => {
					railDragRef.current.on = false;
					document.body.removeAttribute("data-dsh-ssb-rail-dragging");
					try { localStorage.setItem("ssb-rail-w", String(railWRef.current)); } catch {}
			...
			(0, react.useEffect)(() => {
				document.documentElement.style.setProperty("--ssb-rail-width", `${railW}px`);
				return () => { document.documentElement.style.setProperty("--ssb-rail-width", "48px"); };
			}, [railW]);
			// [stage B 2026-08-31] trayOpen state removed: the preset-tab shortcuts
			// now live on the permanent right activity rail, so the old
			// expandable top-right tray (and its panelOpen-fold effect) is gone.
			const service = (typeof ctx.get === "function" ? ctx.get("betterSidebar") : void 0) ?? ctx.betterSidebar;
			const scope = sessionId === void 0 ? void 0 : { sessionId };
			// [stage A 2026-08-31] Left-sidebar (session rail) toggle. Core left-rail
			// collapse is driven by the client layout service (ctx.layout from
			// dsh-client-ui-layout's LayoutController), resolved with the same
			// pattern as the betterSidebar service above.
			const layout = (typeof ctx.get === "function" ? ctx.get("layout") : void 0) ?? ctx.layout;
			/**
			* [stage A 2026-08-31] The layout service exposes only toggleSidebar() /
			* openDetails() / closeDetails() with no observable state, so this button
			* carries no active highlight (active is always false). It disables
			* itself when the layout service is missing; it reuses the shared
			* ssb-btn styling.
			*/
			const leftSidebarButton = (key) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
				label: "左侧栏打开/关闭",
				side: "bottom",
				delayMs: 500,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					className: "ssb-btn",
					disabled: layout === void 0 || disabled,
					title: "左侧栏打开/关闭",
					"aria-label": "左侧栏打开/关闭",
					onClick: () => {
			...
			const findTabByType = (0, react.useCallback)((type) => {
				if (state === void 0) return void 0;
				for (const leaf of allLeaves(state.splits).concat(allLeaves(state.bottomSplits))) {
					const tab = leaf.tabs.find((candidate) => candidate.type === type);
					if (tab !== void 0) return { leaf, tab };
				}
				return void 0;
			}, [state]);
			const isFrontAndActive = (0, react.useCallback)((hit) => {
				if (hit === void 0 || state === void 0 || !state.panelOpen) return false;
				return treeOf(state, hit.leaf.id) === "splits" && hit.leaf.active === hit.tab.id && (state.activePane ?? "") === hit.leaf.id;
			}, [state]);
			/** Zone A — canvas wake/close (same calls as the Play button chain). */
			// [removed 2026-08-30 by user request] the tray's "画布" shortcut:
			// the preview tab stays reachable through the sidebar + menu and
			// link takeover; the always-visible tray button is gone.
			/*
			const onCanvasClick = () => {
				if (service === void 0 || state === void 0) return;
				const hit = findTabByType("webpreview");
				if (hit !== void 0 && isFrontAndActive(hit)) service.closeTab(hit.tab.id, scope);
				else service.openTab({ type: "webpreview", path: hit?.tab.path ?? "preview" }, scope);
			};
			*/
			/** Zone B — one preset tab type per shortcut button. */
			const presetTabs = service === void 0 || disabled ? [] : service.getTabs().filter((descriptor) => !descriptor.hidden && descriptor.id !== void 0 && descriptor.id !== "webpreview" && !descriptor.id.startsWith("agent:") && service.isTabEnabled(descriptor.id) !== false).sort((a, b) => (a.order ?? 100) - (b.order ?? 100));
			const onPresetClick = (descriptor) => {
				if (service === void 0 || state === void 0) return;
				const hit = findTabByType(descriptor.id);
				if (hit !== void 0 && isFrontAndActive(hit)) {
					store.reduce(togglePanel);
					return;
				}
				if (!state.panelOpen) store.reduce((s) => s.panelOpen ? s : togglePanel(s));
			...
			const canvasHit = disabled ? void 0 : findTabByType("webpreview");
			const canvasActive = isFrontAndActive(canvasHit);
			const renderButton = (key, label, active, icon, onClick) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
				label,
				side: "bottom",
				delayMs: 500,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
			...
			/** Zone C buttons shared by the collapsed rail and the expanded tray:
			* the collapsed state keeps the two panel toggles directly clickable
			* (user request 2026-08-29) so the most frequent panel actions never
			* require expanding the tray first. */
			const panelButton = (key) => renderButton(key, state !== void 0 && state.panelOpen ? t("collapse") : t("expand"), state !== void 0 && state.panelOpen, /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconPanelRightOutline16, {}), () => {
				store.reduce(togglePanel);
			});
			const bottomButton = (key) => !narrow && renderButton(key, state !== void 0 && state.bottomOpen ? t("collapseBottomPanel") : t("expandBottomPanel"), state !== void 0 && state.bottomOpen, /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconPanelBottomOutline16, {}), () => {
				store.reduce(toggleBottomPanel);
				/** Opening into an empty bottom tree would animate in with no
				* operable content (the first-expansion auto-terminal effect fires
				* only once per session). Seed a terminal tab when opening with no
				* tabs in the bottom tree — same prefs/enable gates, same openTab
				* chain as that effect. */
				if (state === void 0 || state.bottomOpen) return;
				const hasTabs = allLeaves(state.bottomSplits).some((leaf) => leaf.tabs.length > 0);
				if (hasTabs || service === void 0) return;
			...
			/**
			* [stage B 2026-08-31] Permanent right-edge activity rail (VS Code Activity
			* Bar style): one always-visible shortcut per enabled preset tab. Icon =
			* descriptor.icon(16) for functional icons, the icon node itself otherwise,
			* or the title's first glyph as a text block; active highlight follows
			* isFrontAndActive(findTabByType(descriptor.id)) with a 2px blue left
			* indicator. Hidden on narrow (<768px) via CSS media query. The bottom of
			* the rail is intentionally left empty (no margin-top:auto extras).
			*/
			const railWide = railW > 56;
			const activityRail = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "ssb-actrail",
				"aria-orientation": "vertical",
				children: [
					// [stage C3 2026-08-31] drag-to-expand: a left-edge resize
					// handle widens the rail (icon-only <-> icon+text rows) instead
					// of a hover auto-expand.
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "ssb-actrail-resize",
						onPointerDown: onRailDragStart,
						title: "拖拽展开/收起活动栏",
						"aria-label": "拖拽展开/收起活动栏"
					}),
					...presetTabs.map((descriptor) => {
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
						...
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							type: "button",
							className: railWide ? "ssb-actrail-btn ssb-actrail-btn-row" : "ssb-actrail-btn",
							"data-active": isFrontAndActive(hit) ? "1" : void 0,
							disabled,
							title: title,
							"aria-label": title,
							onClick: () => {
								onPresetClick(descriptor);
							},
							children: [
								icon ?? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "ssb-glyph",
									children: String(title).slice(0, 1)
								}),
								// [stage B2 2026-08-31] wide state adds a label; ellipsis
								// via .ssb-actrail-btn-row .ssb-actrail-label.
								railWide ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "ssb-actrail-label",
									children: title
								}) : null
							]
						})
					}, `ssb-actrail-${descriptor.id}`);
				})]
			});
			/** [stage B 2026-08-31] The top-right cluster is now just the three-zone
			* switch bar [左栏][右面板][底部] — the preset-tab shortcuts and the
			* fold/unfold chevron moved to the activity rail / were removed. */
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				activityRail,
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "ssb-root",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "ssb-tray",
						children: [
							// [stage A 2026-08-31] left-rail toggle first, so the
							// cluster reads [左栏][右面板][底部].
							leftSidebarButton("ssb-left-c"),
							panelButton("ssb-panel-c"),
							bottomButton("ssb-bottom-c")
						]
					})
				})
			] });
		...
		function Sidebar(props) {
			...
			*/
			...
			/* fork: shell-edge junction drag (panel width × internal col divider). */
			const edgeDrag = (0, react.useRef)({ startX: 0, startY: 0, startWidth: 0, splitId: "", index: 0, panelH: 1 });
			const [draggingEdge, setDraggingEdge] = (0, react.useState)(false);
			const anyDragging = draggingWidth || draggingBottom || draggingCorner || draggingEdge;
			(0, react.useEffect)(() => {
				draggingRef.current = anyDragging;
				if (!anyDragging) measureCenter();
			}, [anyDragging, measureCenter]);
			// [stage B2 2026-08-31] drag cap follows the rail width.
			const clampWidth = (width) => Math.min(Math.max(280, Math.round(width)), Math.max(280, window.innerWidth - ssbRailWidthPx()));
			const clampHeight = (height) => Math.min(Math.max(120, Math.round(height)), Math.max(120, window.innerHeight - 280));
			/** Fixed panels follow the pointer independently. The app-shell layout
			* variables are deliberately untouched until pointer-up: changing them
			* reflows the entire conversation transcript and causes frame skips. */
			const applyDrag = (width, height) => {
				...
				// [audit 2026-08-31] the bottom panel spans the CENTER column only:
				// centerRect.right already excludes the rail + panel give-way (via
				// --dsh-sidebar-width), so the right offset is centerRect.right
				// alone. The earlier `- ssbRailWidthPx()` double-counted the rail
				// (sliding the bottom panel under the rail when the panel closed).
			...
			*  pointer up applies the final clamped values). */
			...
			(0, react.useEffect)(() => {
				// [stage B2 2026-08-31] the center yields the rail's current width
				// (non-narrow); the open panel adds its (rail-aware) width.
				const railW = ssbRailWidthPx();
				const width = !narrow
					? railW + (snapshot.state?.panelOpen === true ? Math.min(snapshot.state.width, Math.max(0, window.innerWidth - railW)) : 0)
					: 0;
			...
			*/
			...
			if (state === void 0 || sessionId === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SecondarySideBarToolbar, {
				ctx,
				store,
				state,
				sessionId,
			...
			*/
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SecondarySideBarToolbar, {
					ctx,
					store,
					state,
					sessionId,
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					// [stage B2 2026-08-31] inline width capped rail-width-aware.
					style: { width: narrow ? "100vw" : Math.min(state.width, window.innerWidth - ssbRailWidthPx()) },
					...
					children: [
						...
						/* fork: shell-edge junctions — 4-way grips where the panel's
						 * left edge crosses the workbench's left-edge col dividers
						 * (conversation | right panel × top/bottom pane). */
						!narrow && state.panelOpen && edgeDividerFracs(state.splits, 0, 1, []).map((spot) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: clsx(sidebar_module_css_default.panelEdgeJunction, draggingEdge && sidebar_module_css_default.junctionHandleActive),
							style: { top: `${(spot.topFrac * 100).toFixed(4)}%` },
							"data-dragging": draggingEdge || void 0,
							"aria-hidden": "true",
							key: `${spot.splitId}:${spot.index}`,
							onPointerDown: (event) => {
								event.preventDefault();
								event.currentTarget.setPointerCapture(event.pointerId);
								document.body.setAttribute("data-dsh-sidebar-dragging", "");
								...
								edgeDrag.current = {
									...
									startWidth: state.width,
									splitId: spot.splitId,
									index: spot.index,
									panelH: box === void 0 || box.height < 1 ? 1 : box.height
								};
							...
							onPointerMove: (event) => {
								...
								const width = clampWidth(drag.startWidth + (drag.startX - event.clientX));
								const height = state.bottomOpen ? Math.min(state.bottomHeight, window.innerHeight) : 0;
								scheduleDrag(width, height);
								const dy = event.clientY - drag.startY;
								if (dy !== 0) actions.resizeSplit(drag.splitId, drag.index, dy / drag.panelH);
							...
							onPointerUp: (event) => {
								...
								stopDragScheduling();
								store.reduce((s) => setWidth(s, drag.startWidth + (drag.startX - event.clientX)));
				...
				!narrow && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					style: {
						...
						// [audit 2026-08-31] bottom panel spans the CENTER column: its
						// right edge = centerRect.right (the give-way offset already
						// includes the rail + panel, so no extra -railW).
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							onPointerMove: (event) => {
								...
								const height = clampHeight(startHeight + (startY - event.clientY));
								scheduleDrag(Math.min(state.width, window.innerWidth - ssbRailWidthPx()), height);
							},
							onPointerUp: (event) => {
								if (!event.currentTarget.hasPointerCapture(event.pointerId)) return;
								event.currentTarget.releasePointerCapture(event.pointerId);
		...
		*/
		function apply(ctx) {
			...
			try {
				resetChunks();
				const dshModuleSystem = ctx.get?.("modules");
				bindModuleSystem(dshModuleSystem);
				ctx.effect(() => {
					let disposed = false;
					let root;
					let host;
