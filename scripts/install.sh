#!/usr/bin/env bash
# =============================================================================
# @all3cn/dsh-better-sidebar-n23 一键安装脚本（官方 CLI 方式，macOS / Linux / Windows Git Bash）
#
...
#   --restart   装完后通过 DSH_RESTART_CMD 执行受控优雅重启；缺省仅提示。
...
# - 回滚：dsh plugin --profile web remove @all3cn/dsh-better-sidebar-n23，或把 profile 依赖
#   改回 "@all3cn/dsh-better-sidebar-n23": "link:<路径>" 再 pnpm install。
# =============================================================================
set -euo pipefail
...
for arg in "$@"; do
  if [ "$arg" = "-h" ] || [ "$arg" = "--help" ]; then
    cat <<'EOF'
@all3cn/dsh-better-sidebar-n23 一键安装 / 依赖修复脚本
...
用法：
  ...
  --restart    装完后通过 DSH_RESTART_CMD 执行受控优雅重启（未配置时仅提示）
  --dry-run    只打印将要执行的操作，不写任何文件

环境变量（可省略）：DSH_HOME（默认 ~/.dsh）、REGISTRY（npm 源）、DSH_CMD（dsh 命令）、DSH_RESTART_CMD（受控优雅重启命令）
EOF
    exit 0
  fi
done
...
DSH_CMD="${DSH_CMD:-dsh}"
# Optional command used only when --restart is explicitly requested. It must
# enforce SIGTERM-first graceful stopping and a bounded wait window itself.
DSH_RESTART_CMD="${DSH_RESTART_CMD:-}"
...
if [ "$DRY_RUN" = true ]; then
  ...
  if [ "$RESTART" = true ]; then say "[dry-run] 步骤 5：执行 DSH_RESTART_CMD（必须实施 SIGTERM + 30 秒优雅等待策略）"; else say "[dry-run] 步骤 5：提示用户通过受控重启入口重启 DSH"; fi
  exit 0
fi
...
# 步骤 4：重启。禁止直接调用 pm2 等无法保证优雅等待窗口的重启器。
if [ "$RESTART" = true ]; then
  if [ -n "$DSH_RESTART_CMD" ]; then
    say "执行受控优雅重启（SIGTERM 后最多等待 30 秒）..."
    if ! "$DSH_RESTART_CMD"; then
      warn "受控优雅重启失败；请检查重启器输出后再处理。"
    fi
  else
    warn "未配置 DSH_RESTART_CMD；为保证 SIGTERM + 30 秒优雅等待策略，未执行自动重启。"
  fi
else
  say "下一步：通过本机受控重启入口重启 DSH，并硬刷新（Cmd/Ctrl+Shift+R）使新副本生效。"
fi
