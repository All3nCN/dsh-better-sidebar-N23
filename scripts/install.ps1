...
#   -Restart    为避免绕过优雅等待策略，不自动调用 pm2；仅提示使用本机受控重启入口。会断开当前页面会话。
...
if ($DryRun) {
  ...
  if ($Restart) { Say '[dry-run] 步骤 5：提示用户使用本机受控优雅重启入口（SIGTERM + 30 秒等待）' } else { Say '[dry-run] 步骤 5：提示用户通过受控重启入口重启 DSH' }
  exit 0
}
...
# 步骤 5：重启提示。禁止直接调用 pm2 等无法保证优雅等待窗口的重启器。
if ($Restart) {
  Warn '为保证 SIGTERM 后至少 30 秒的优雅等待策略，未执行自动重启。请使用本机受控重启入口。'
} else {
  Say '下一步：通过本机受控重启入口重启 DSH，并硬刷新（Ctrl+Shift+R / Cmd+Shift+R）使新副本生效。'
}
