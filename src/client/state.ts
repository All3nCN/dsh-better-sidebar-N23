...
/**
 * fork: shell-edge dividers — horizontal (col-split) dividers inside the tree
 * whose span touches the panel's LEFT edge. Each crossing of such a divider
 * with the panel edge is a conversation|panel×pane junction that gets a 4-way
 * handle. Walk: col splits lay children full-width (recurse every child with
 * its y-window); row splits only expose children[0] to the left edge.
 */
export function edgeDividerFracs(
  node: SplitNode,
  top: number,
  height: number,
  out: { splitId: string, index: number, topFrac: number }[],
): { splitId: string, index: number, topFrac: number }[] {
  if (node.kind !== 'split') return out
  if (node.dir === 'col') {
    ...
    edgeDividerFracs(node.children[0]!, top, height, out)
  }
  return out
}
