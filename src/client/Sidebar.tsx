...
import {
  BOTTOM_MIN, PANEL_MIN, agentUuidOf, edgeDividerFracs, firstLeaf, isAgentTabId, leafWithTab, migrateBottomTabs, moveTab, moveTabToEdge, openDiffTab,
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  const edgeDrag = useRef({ startX: 0, startY: 0, startWidth: 0, splitId: '', index: 0, panelH: 1 })
  ...
  /**
   * Apply a drag size without touching React state, the store, or the app-shell
   * layout variables. Changing --dsh-sidebar-width/height reflows the entire
   * conversation transcript (message wrapping, ResizeObservers and scroll
   * anchoring) and was the source of visible main-pane frame skips. The fixed
   * panels therefore follow the pointer independently; pointer-up commits one
   * exact size, producing a single main-pane layout sync rather than dozens.
   */
  const applyDrag = (width: number, height: number): void => {
    ...
    // drag write-only (no React re-render or main-pane reflow mid-drag).
    ...
    <>
      ...
      */}
      ...
      >
          {!narrow && (
            <div
              ...
              onPointerDown={(event) => {
                ...
                // React applies the attribute after this event. Do it now so
                // #root never begins a drag with its expand/collapse margin
          ...
          {/* fork: shell-edge junctions — 4-way grips where the panel's left
              edge crosses the workbench's left-edge col dividers
              (conversation | right panel × top/bottom pane). */}
          {!narrow && state.panelOpen && edgeDividerFracs(state.splits, 0, 1, []).map(spot => (
            <div
              key={`${spot.splitId}:${spot.index}`}
              className={clsx(css.panelEdgeJunction, draggingEdge && css.junctionHandleActive)}
              style={{ top: `${(spot.topFrac * 100).toFixed(4)}%` }}
              data-dragging={draggingEdge || undefined}
              aria-hidden="true"
              onPointerDown={(event) => {
                event.preventDefault()
                event.currentTarget.setPointerCapture(event.pointerId)
                ...
                edgeDrag.current = {
                  ...
                  index: spot.index,
                  panelH: box === undefined || box.height < 1 ? 1 : box.height,
                }
