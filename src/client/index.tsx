...
import { resetChunks, bindModuleSystem, type ChunkModuleSystem } from './chunk-loader.ts'
...
export function apply(ctx: Context): void {
  ...
  try {
    ...
    resetChunks()
    // rc.8 removed the window.__DSH_MODULES__ page global: the client module
    // system is now the `modules` Cordis service (kernel-built before cordis
    // exists, enrolled by dsh-client-modules). ctx.get reads it optionally —
    // no inject declaration, so an rc.6-era host without the service keeps
    // the legacy-global path inside the chunk loader.
    const moduleSystem = (ctx as Context & { get?: (name: string) => unknown })
      .get?.('modules') as ChunkModuleSystem | undefined
    bindModuleSystem(moduleSystem)
    ctx.effect(() => {
      let disposed = false
