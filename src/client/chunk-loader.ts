...
/**
 * Module system bound from the plugin context (rc.8+ hosts). rc.8 removed the
 * `window.__DSH_MODULES__` page global — the shell kernel now builds the
 * ClientModuleSystem before cordis exists and enrolls it as the `modules`
 * service (`ctx.reflect.provide("modules", …)` in dsh-client-modules), so a
 * plugin reads it through its context instead of a page global.
 */
let boundModules: ChunkModuleSystem | undefined

/**
 * Bind the client module system resolved from the plugin context; called at
 * every client activation (HMR re-activation re-binds the current instance).
 * `undefined` un-binds — an rc.6-era host without the service keeps the
 * legacy-global fallback in {@link moduleSystem}.
 */
export function bindModuleSystem(mod: ChunkModuleSystem | undefined): void {
  boundModules = mod
}
