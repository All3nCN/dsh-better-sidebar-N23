...
export function TerminalView(props: { scope: SessionScope; tabId: string; store: SidebarStore }) {
  ...
  useEffect(() => {
    ...
    let failures = 0
    // fork: first-wake blank fix — deferred re-fit timers (see openWhenSized).
    ...
    const cancelOpen = openWhenSized(host, () => {
      ...
      // fork: first-wake blank fix — xterm's char measurement is async; when
      // the pane was hidden (display:none tab or an off-screen panel) at
      // mount, the first fit ran against stale/zero cell metrics and the
      // canvas renders blank until a later resize event. Re-fit over the
      // next frames; no-op fits are cheap once dimensions match. Cleared on
      // unmount below.
      for (let i = 1; i <= 4; i++) fitTimers.push(window.setTimeout(() => {
        if (closed) return
        try {
          fit.fit()
          sendResize()
