 ...
 * opposite-direction nested split's divider (three panes meeting at one
 * point), a small circular `cursor: move` grip lets one drag feed BOTH
 * resizeSplit routes: x delta → the outer split, y delta → the nested one
 * (mirrored for column splits). Deltas stay incremental like Divider for
 * the same run-away-prevention reason.
 */
function JunctionHandle(props: {
  style: CSSProperties
  onResizeX: (deltaFrac: number) => void
  onResizeY: (deltaFrac: number) => void
}) {
  const { style, onResizeX, onResizeY } = props
  // handle → junction layer → split container box (for px→frac normalization).
  const last = useRef({ x: 0, y: 0, w: 1, h: 1 })
  const [dragging, setDragging] = useState(false)

  return (
    <div
      className={clsx(css.junctionHandle, dragging && css.junctionHandleActive)}
      style={style}
      aria-hidden="true"
      onPointerDown={(event) => {
        event.preventDefault()
        ...
        last.current = {
          x: event.clientX,
          y: event.clientY,
          w: box === undefined ? 1 : box.width,
          h: box === undefined ? 1 : box.height,
        }
        setDragging(true)
      }}
      onPointerMove={(event) => {
...
/**
 * fork: junction handles for one divider of a split node. `neighbor` is the
 * adjacent child that is an opposite-direction split; each of its internal
 * dividers ends exactly on this node's divider, producing one 4-way handle
 * per crossing.
 */
function SplitJunctions(props: {
  node: SplitNode & { kind: 'split' }
  index: number
  neighbor: SplitNode & { kind: 'split' }
  ...
  for (let k = 0; k < neighbor.children.length - 1; k++) {
    ...
    handles.push(
      <JunctionHandle
        key={`${neighbor.id}:${k}`}
        style={isRow
          ? { left: `${(mainFrac * 100).toFixed(4)}%`, top: `${((acc / nestedTotal) * 100).toFixed(4)}%` }
          : { top: `${(mainFrac * 100).toFixed(4)}%`, left: `${((acc / nestedTotal) * 100).toFixed(4)}%` }}
        onResizeX={(deltaFrac) => {
          if (isRow) actions.resizeSplit(node.id, index - 1, deltaFrac)
          else actions.resizeSplit(neighbor.id, k, deltaFrac)
        }}
        onResizeY={(deltaFrac) => {
          if (isRow) actions.resizeSplit(neighbor.id, k, deltaFrac)
          else actions.resizeSplit(node.id, index - 1, deltaFrac)
        }}
      />,
    )
  }
...
function NodeView(props: {
  ...
  const isRow = node.dir === 'row'
  return (
    <div className={clsx(css.split, isRow ? css.splitRow : css.splitCol, css.splitPositioned)}>
      {node.children.map((child, index) => (
        <Fragment key={child.id}>
          ...
          {/* fork: three-pane junction — when either side of this divider is an
              opposite-direction split, add 4-way handles at the crossings. */}
          {index > 0 && (() => {
            const perpendicular = (candidate: SplitNode): (SplitNode & { kind: 'split' }) | null =>
              candidate.kind === 'split' && candidate.dir !== node.dir ? candidate : null
            const neighbor = perpendicular(node.children[index]!) ?? perpendicular(node.children[index - 1]!)
            return neighbor === null ? null : (
              <SplitJunctions node={node} index={index} neighbor={neighbor} actions={actions} />
            )
          })()}
          <div
